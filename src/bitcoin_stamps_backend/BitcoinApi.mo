...
module {
  ...
  type GetBlockHeadersResult = Types.GetBlockHeadersResult;
  type GetBlockHeadersRequest = Types.GetBlockHeadersRequest;
  ...
  let SEND_TRANSACTION_BASE_COST_CYCLES : Cycles = 50_000_000_000;
  let SEND_TRANSACTION_COST_CYCLES_PER_BYTE : Cycles = 200_000_000;
  ...
  type ManagementCanisterActor = actor {
      ...
      bitcoin_get_block_headers : GetBlockHeadersRequest -> async GetBlockHeadersResult;
  };
  ...
  /// NOTE: Relies on the `bitcoin_get_utxos` endpoint.
  /// See https://internetcomputer.org/docs/current/references/ic-interface-spec/#ic-bitcoin_get_utxos
  public func get_block_headers(network : Network, start_height: Nat32, end_height: ?Nat32) : async GetBlockHeadersResult {
